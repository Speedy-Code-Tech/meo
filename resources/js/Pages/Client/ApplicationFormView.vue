...
const routeName = isNotificationPage == true ? 'notifications.getrecord.index' : 'my-application-forms-record';
...
<template>
	...
	<div class="px-6 card">
		...
		>
			<div>
				Status : <span class="font-bold uppercase">{{ form.status }}</span>
			</div>
			Remarks : <span class="font-bold uppercase">{{ form.remarks }}</span>
		</div>
		...
		<div class="!p-2 mt-2">
			...
			<Map @update:show="toggleMaps(false)"
				:show="showMaps"
				:isView="true"
				:lat="form.latitude"
				:lng="form.longitude"
